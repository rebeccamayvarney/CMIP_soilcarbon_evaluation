...
# My functions
from rmv_cmip_analysis import combine_netCDF_cmip5, open_netCDF, regrid_model, numpy_to_cube
...
rmse_array = ma.zeros([len(cmip5_models)])
...
for model_i in range(n_models):
    ...
    upper_bound = 70  # must be between -90 and 90
    ...
    rmse_cSoil = np.square(np.subtract(soil_carbon_data_masked, observational_cSoil_masked))
    rmse_cSoil_cube = numpy_to_cube(rmse_cSoil, regrid_cube_masked, 2)
...
#%%
# Save figure
fig.savefig('paper_figures/CMIP5_cSoildifference_maps_v1', bbox_inches='tight')
plt.close()

#save
np.save("rmse_NL_cSoil_cmip5_array.npy", rmse_array.data)
