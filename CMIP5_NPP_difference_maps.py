...
#    npp_data = ma.masked_where(np.logical_or(npp_data < 1e-8, npp_data > 1e8), npp_data)
    ...
    npp_cube = combine_netCDF_cmip5('/home/rmv203/DATA/cmip5_data/rh_Lmon_'+model+'_historical*', 'heterotrophic_respiration_carbon_flux', model)
    npp_cube = open_netCDF(npp_cube)        
    npp_cube = select_time(npp_cube, 1995, 2005)
    npp_cube = time_average(npp_cube)
    npp_cube = regrid_model(npp_cube, regrid_cube)
...
#%%
# Save figure
fig.savefig('figA04', bbox_inches='tight')
plt.close()


#save
#np.save("rmse_npp_cmip5_array.npy", rmse_array.data)